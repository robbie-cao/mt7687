...
#include "ble_smtcn_app.h"  //ble wifi smart connection
#include "ble_smtcn.h"
#endif
...
bt_bool_t sc_only, sc_only_default = BT_FALSE;
...
static uint8_t ut_app_reset_global_config_flag = BT_TRUE;
static uint8_t ut_app_reset_flash_flag = BT_TRUE;
bt_status_t (*ut_app_callback)(bt_msg_type_t, bt_status_t, void *) = NULL;
/* End of default configuration. */

bt_bool_t bt_app_advertising = BT_FALSE;
bt_bool_t bt_app_scanning = BT_FALSE;
bt_bool_t bt_app_connecting = BT_FALSE;
bt_bool_t bt_app_wait_peer_central_address_resolution_rsp = BT_FALSE;
...
static bool g_bt_hci_log_enable = BT_FALSE;
...
bt_status_t bt_app_io_callback(void *input, void *output)
{
    ...
    if (UT_APP_CMP("?")) {
        ...
        g_bt_hci_log_enable = BT_TRUE;
    }
    else if (UT_APP_CMP("hci off")) {
        g_bt_hci_log_enable = BT_FALSE;
    }
    else if (UT_APP_CMP("reset config off")) {
        ut_app_reset_global_config_flag = BT_FALSE;
    }

    else if (UT_APP_CMP("reset flash off")) {
        ut_app_reset_flash_flag = BT_FALSE;
    }
    /* Usage: set pts_addr [pts address]*/
        ...
        ut_app_reset_global_config_flag = BT_TRUE;
    }

    else if (UT_APP_CMP("reset flash on")) {
        ut_app_reset_flash_flag = BT_TRUE;
    }
    ...
    /* update peer Central Address Resolution supporting */
        ...
        bt_app_wait_peer_central_address_resolution_rsp = BT_TRUE;
    }
   ...
   */
        ...
        adv_enable.advertising_enable = BT_TRUE;
...
bt_status_t bt_app_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
{
    ...
    case BT_POWER_ON_CNF:
        ...
        bt_app_advertising = BT_FALSE;
        bt_app_scanning = BT_FALSE;
        bt_app_connecting = BT_FALSE;
        break;
    case BT_GAP_LE_LOCAL_CONFIG_REQ_IND:
        BT_COLOR_SET(BT_COLOR_RED);
        BT_LOGI("APP", "BT_GAP_LE_LOCAL_CONFIG_REQ_IND %s",
            (status == BT_STATUS_SUCCESS)? "Success":"Failed");
        BT_COLOR_SET(BT_COLOR_WHITE);
        ((bt_gap_le_local_config_req_ind_t *)buff)->local_key_req = &local_key_req;
        ((bt_gap_le_local_config_req_ind_t *)buff)->sc_only_mode_req = sc_only;
        break;
    ...
    case BT_GAP_LE_CONNECT_IND:
    {
        ...
        if (status == BT_STATUS_SUCCESS) {
            ...
            conn_interval = connection_ind->conn_interval * 1.25;
        #ifdef BLE_THROUGHPUT
            if (enable_dle) {
                bt_hci_cmd_le_set_data_length_t data_length;
    ...
    case BT_GAP_LE_CONNECTION_UPDATE_IND:
        ...
        BT_COLOR_SET(BT_COLOR_WHITE);
        break;   
    case BT_GAP_LE_CONNECTION_UPDATE_REQ_IND:
    {
        BT_COLOR_SET(BT_COLOR_RED);
        BT_LOGI("APP", "BT_GAP_LE_CONNECTION_UPDATE_REQ_IND %s",
            (status == BT_STATUS_SUCCESS)? "Success":"Failed");
        BT_COLOR_SET(BT_COLOR_BLUE);

        bt_gap_le_connection_update_req_ind_t *req_ind = (bt_gap_le_connection_update_req_ind_t *)buff;
        BT_LOGI("APP", "interval_min=0x%04x",req_ind->param_req.interval_min);
        BT_LOGI("APP", "interval_max=0x%04x",req_ind->param_req.interval_max);
        BT_LOGI("APP", "slave_latency=0x%04x",req_ind->param_req.slave_latency);
        BT_LOGI("APP", "timeout_multiplier=0x%04x",req_ind->param_req.timeout_multiplier);
        //req_ind->param_req.interval_max = req_ind->param_req.interval_min+40 <= 0x0c80?(req_ind->param_req.interval_min+40):req_ind->param_req.interval_max;
        req_ind->accept_req = BT_HCI_ACCEPT;
        BT_COLOR_SET(BT_COLOR_WHITE);
        break;
    }    
    case BT_GAP_LE_BONDING_INFO_REQ_IND:
        BT_COLOR_SET(BT_COLOR_RED);
        BT_LOGI("APP", "BT_GAP_LE_BONDING_INFO_REQ_IND %s",
            (status == BT_STATUS_SUCCESS)? "Success":"Failed");
        BT_COLOR_SET(BT_COLOR_WHITE);
        ...
        break;
    case BT_GAP_LE_BONDING_START_IND:
        BT_COLOR_SET(BT_COLOR_RED);
        BT_LOGI("APP", "BT_GAP_LE_BONDING_START_IND %s",
            (status == BT_STATUS_SUCCESS)? "Success":"Failed");
        BT_COLOR_SET(BT_COLOR_BLUE);
        BT_LOGI("APP", "lost_bond(%d)", ((bt_gap_le_bonding_start_ind_t *)buff)->is_previously_bonded);
        BT_COLOR_SET(BT_COLOR_WHITE);
        ((bt_gap_le_bonding_start_ind_t *)buff)->pairing_config_req = pairing_config_req;
        break;
        ...
        BT_COLOR_SET(BT_COLOR_WHITE);
        {
            ...
            if (ind->method & BT_GAP_LE_SMP_PASSKEY_DISPLAY_MASK) {
                ...
                if (nc_value_correct[0]!='n' && nc_value_correct[0]!='N') {
                    rsp.nc_value_matched = BT_TRUE;
                } else {
                    rsp.nc_value_matched = BT_FALSE;
                }
...
#endif
    /* GATT */
    case BT_GATTS_CHARC_EXECUTE_WRITE:
        {
            BT_COLOR_SET(BT_COLOR_RED);
            BT_LOGI("APP", "BT_GATTS_CHARC_EXECUTE_WRITE %s",
                (status == BT_STATUS_SUCCESS)? "Success":"Failed");
            BT_COLOR_SET(BT_COLOR_WHITE);

            bt_gatts_execute_write_req_t *req = (bt_gatts_execute_write_req_t*) buff;
            status = bt_gatt_service_execute_write(req->handle, req->flag);
        }
        break;
    case BT_GATTS_CHARC_AUTHORIZATION_CHECK:
        {
            bt_gatts_authorization_check_req_t *req = (bt_gatts_authorization_check_req_t*) buff;
    ...
    case BT_GATTC_READ_USING_CHARC_UUID:
        {
            if (bt_app_wait_peer_central_address_resolution_rsp == BT_TRUE){
                bt_app_wait_peer_central_address_resolution_rsp = BT_FALSE;
                BT_COLOR_SET(BT_COLOR_RED);
