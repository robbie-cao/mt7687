...
extern bt_bool_t bt_app_advertising;
extern bt_bool_t bt_app_scanning;
extern bt_bool_t bt_app_scanning;
...
bt_status_t bt_app_gap_io_callback(void *input, void *output)
{
    ...
    /* Usage: gap wl add [0:public / 1:random] [bt address].
       Note:  default use #lt_addr_type and #lt_addr */
        ...
        bt_hci_cmd_le_add_device_to_white_list_t device;
        if (strlen(cmd) >= 11) {
            ...
            if (addr_type != 0 && addr_type!= 1) {
                ...
                device.address.type = addr_type;
                memcpy(device.address.addr, addr, sizeof(addr));
                bt_gap_le_set_white_list(BT_GAP_LE_ADD_TO_WHITE_LIST, &(device.address));
            }
        } else {
    ...
    /* Usage: gap wl remove [0:public / 1:random] [bt address].
       Note:  default use #lt_addr_type and #lt_addr */
        ...
        bt_hci_cmd_le_remove_device_from_white_list_t device;
        if (strlen(cmd) >= 14) {
            ...
            if (addr_type != 0 && addr_type!= 1) {
                ...
                device.address.type = addr_type;
                memcpy(device.address.addr, addr, sizeof(addr));
                bt_gap_le_set_white_list(BT_GAP_LE_REMOVE_FROM_WHITE_LIST, &(device.address));
            }
        } else {
            //device.address_type = lt_addr_type;
            //memcpy(device.address, lt_addr, sizeof(lt_addr));
