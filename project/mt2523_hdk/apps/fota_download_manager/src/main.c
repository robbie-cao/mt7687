...
#include "syslog.h"
//log_bt_create(PRINT_LEVEL_INFO);
...
int __io_putchar(int ch)
{
    ...
    /* e.g. write a character to the EVAL_COM1 one at a time */
...
#if 1 /* remove bt_task, cmd_task, unnecessary code */
void bt_task(void *arg)
{
    ...
    while (1) {
        if (cmd_received == 1) {
            ...
            bt_console_cmd((const char*)console_cmd);
            console_cmd[0] = '\0';
            cmd_received = 0;
        }
...
#endif /* remove bt_task, cmd_task, unnecessary code */

void bt_gap_common_callback(bt_event_t event_id, void *param)
{
    log_bt_info("Received event %d", event_id);
    switch (event_id) {
...
{
    //if (g_p_gap_get_local_address != NULL) {
    //    return g_p_gap_get_local_address(local_addr);
    //}
    //	return -1;
    local_addr->address[0] = 0x11;
    local_addr->address[1] = 0x22;
    local_addr->address[2] = 0x33;
    local_addr->address[3] = 0x44;
    local_addr->address[4] = 0x55;
    local_addr->address[5] = 0x66;
    return BT_STATUS_SUCCESS;
}

bt_status_t bt_gap_get_local_name(char *name)
{
    strcpy(name, "FOTA_TEST");
    return BT_STATUS_SUCCESS;
}
...
int main(void)
{
    ...
    //xTaskCreate(cmd_task, "cmd_task", 2048, NULL, 1, &xCreatedTask1);
    xTaskCreate(fota_task, "fota_task", 1024, NULL, 1, &xCreatedTask2);
