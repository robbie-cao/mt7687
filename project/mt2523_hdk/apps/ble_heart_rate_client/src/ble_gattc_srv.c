...
void  *gattc_register(gattc_register_req_struct *req, app_callback_struct *cb)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]Gattc_Register: -- start: count = %d", count);
    ...
    LOG_I(gattc_srv, "[HRSRV]Gattc_Register: -- start: end = %x", user);
    return (void *)user;
}
...
void gattc_notify_scan_info_to_all_user(ble_gap_advertising_ind_t *param)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]scan_info_to_all_user: --start");

    if (!Is_List_Empty(&GATTC(userlist))) {/*notify all user*/

        //LOG_I(gattc_srv, "[HRSRV]scan_info_to_all_user: user is not null");
    ...
    //LOG_I(gattc_srv, "[HRSRV]scan_info_to_all_user: --end");
}
...
void gattc_notify_conn_info_to_all_user(gattc_conn_t *gattc_conn, ble_status_t error_code, uint16_t type)
{
    ...
    // LOG_I(gattc_srv, "[HRSRV]gattc_notify_conn_info_to_all_user: --start:con_id = %d", conn.conn_id);
    if (type) {
        conn_info = ble_gap_get_connection_info(conn.conn_id);
        if (conn_info == NULL) {

            LOG_I(gattc_srv, "[HRSRV]gattc_notify_conn_info_to_all_user: --conn info is null");
            return ;
        }
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_notify_conn_info_to_all_user: --conn = %d, error_code = %d", is_ind, error_code);
        ...
        while (next != &GATTC(userlist)) {
            ...
            if (cb) {
                if (cb->connect_cb) {
                    ...
                    if (type) {
                        memcpy(&peer_addr, &conn_info->peer_addr, sizeof(ble_address_t));
                        cb->connect_cb(&conn, connected, &peer_addr, is_ind);
                    } else {
    ...
    // LOG_I(gattc_srv, "[HRSRV]gattc_notify_conn_info_to_all_user: --end");
}
...
int32_t gattc_search_service(gattc_search_service_req_t *req)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_search_service: -- start: conn_id = %d", conn_id);

    if (p_connect_cntx == NULL) {
        LOG_I(gattc_srv, "[HRSRV]gattc_search_service: -- conn link is null");
        return status;
    }

    service_db = ble_gattc_query_database_is_exist(conn_id);

    //LOG_I(gattc_srv, "[HRSRV]gattc_search_service: -- state = %d", p_connect_cntx->state);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_search_service: --end:  status = %d", status);
...
int32_t gattc_get_incl_service(gattc_search_include_service_req_t *req, gatt_srv_uuid_t *incl_uuid)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_get_Incl_service: --end:  status = %d", status);
...
int32_t gattc_get_char(gattc_get_char_req_t *req, gatt_get_char_result_t *char_info)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_get_char: --start : conn_id = %d", conn_id);
    p_connect_cntx =  ble_gattc_link_info_by_id(conn_id);

    if (p_connect_cntx) {
        //LOG_I(gattc_srv, "[HRSRV]gattc_get_char: --state = %d", p_connect_cntx->state);
        ...
        if (p_connect_cntx->state == GATTC_SEARCH_DONE) {
            ...
            //LOG_I(gattc_srv, "[HRSRV]gattc_get_char: --start_uuid = %x", req->start_char_uuid);
            ...
            if (srv_node) {
                ...
                if (char_node) {
                    ...
                    // LOG_I(gattc_srv, "[HRSRV]gattc_get_char: --result = %d, pro = %d", status,char_info->prop);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_get_char: --end : status = %d", status);
    return status;
}
...
int32_t gattc_get_descr(gattc_get_descr_req_t *req, gatt_uuid_t *descr_info)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_get_descr: --start : conn_id = %d", conn_id);
    p_connect_cntx =  ble_gattc_link_info_by_id(conn_id);

    if (p_connect_cntx) {

        //LOG_I(gattc_srv, "[HRSRV]gattc_get_descr: --state = %d", p_connect_cntx->state);
        ...
        if (p_connect_cntx->state == GATTC_SEARCH_DONE) {
            ...
            if (srv_node) {
                ...
                if (char_node) {
                    ...
                    //LOG_I(gattc_srv, "[HRSRV]gattc_get_descr: --result = %d", status);
                } else {
...
int32_t gattc_read_char(gattc_read_char_req_t *req)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_read_char: -- start: conn_id = %d", conn_id);
    gattc_conn = ble_gattc_link_info_by_id(conn_id);

    if (gattc_conn == NULL) {
        LOG_I(gattc_srv, "[HRSRV]gattc_read_char: --gattc_conn is null or conn_info is wrong");
        return status;
    }

    //LOG_I(gattc_srv, "[HRSRV]gattc_read_char: -- state = %d, type = %d", gattc_conn->state, req->type);
...
int32_t gattc_read_descr(gattc_read_descr_req_t *req)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_read_descr: -- start: conn_id = %d", conn_id);

    if (!gattc_conn) {
        LOG_I(gattc_srv, "[HRSRV]gattc_read_descr: -- gattc_conn is null or connect_info is null");
        return status;
    }

    //LOG_I(gattc_srv, "[HRSRV]gattc_read_descr: -- start: state = %d", gattc_conn->state);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_read_descr: -- end: status = %d", status);
    return status;
}
...
int32_t gattc_write_char(gattc_write_char_req_t *req)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_char: -- start: conn_id = %d", conn_id);

    gattc_conn = ble_gattc_link_info_by_id(conn_id);

    if (!gattc_conn) {

        LOG_I(gattc_srv, "[HRSRV]gattc_write_char: -- gattc_conn is null or connect_info is null");
        return status;
    }

    //LOG_I(gattc_srv, "[HRSRV]gattc_write_char: -- state = %d", gattc_conn->state);
    ...
    if (gattc_conn->state == GATTC_SEARCH_DONE) {
        ...
        if (srv_node) {
            char_node = ble_gattc_find_characteristic_by_uuid(srv_node, req->char_uuid);
            //LOG_I(gattc_srv, "[HRSRV]char node = %x\r\n", char_node);
            if (char_node) {
                ...
                //LOG_I(gattc_srv, "[HRSRV]write status = %d\r\n", status);
            } else {
                status = GATTC_STATUS_CHAR_NOT_FOUND;
            }
        } else {
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_char: -- status = %d", status);
    return status;
}
...
int32_t gattc_write_descr(gattc_write_descr_req_t *req)
{
    ...
    conn_id = req->conn->conn_id;
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_descr: -- start: conn_id = %d", conn_id);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_descr: -- state = %d", conn->state);
        ...
        if (srv_node) {
            ...
            if (char_node) {
                ...
                if (descr_node) {
                    ...
                    write_req.size = req->len;
                    write_req.value = req->value;
                    LOG_I(gattc_srv, "[HRSRV]gattc_write_descr: --handle = %d, len = %d", write_req.handle, write_req.size);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_descr: -- end:status = %d", status);
    return status;
}
...
int32_t gattc_configure_mtu(gattc_cfg_mtu_t *req)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_config_mtu: -- start:conn_id = %d, req_mtu = %d, mtu_size = %d", conn_id, req->mtu_size, mtu_size);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_config_mtu: -- start:conn_id = %d, req_mtu = %d, mtu_size = %d", status);
    return status;
}
...
int32_t gattc_hv_confirm(gattc_hv_confirm_t *req)
{
    ...
    int32_t status = GATTC_STATUS_INVALID;
    conn_id = req->conn->conn_id;
    //LOG_I(gattc_srv, "[HRSRV]gattc_hv_confirm: -- start:conn_id = %d", conn_id);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_hv_confirm: -- end:conn_id = %d", status);
...
static gattc_user_context_t *gattc_service_alloc_user()
{
    ...
    // LOG_I(gattc_srv, "[HRSRV]gattc_service_alloc_user: -- start:");
    if (!Is_List_Empty(&GATTC(freeuserlist))) {
        //LOG_I(gattc_srv, "[HRSRV]gattc_service_alloc_user: free user list is not null");
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_service_alloc_user--end: entry = %x", entry);
...
static void gattc_read_char_descr_value_cb(ble_gattc_read_char_descriptor_rsp_t *param)
{
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_read_value_cb: -- start:conn_id = %d, handle = %d, result =%d", conn_id, handle, param->result);


    service_node = ble_gattc_check_service_handle_range(conn_id, handle);
    if (!service_node) {
        LOG_I(gattc_srv, "[HRSRV]gattc_read_value_cb: -- service_node is null");
        return;
    }
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_read_value_cb: -- type = %d", type);
    ...
    while (&service_node->userlist != user_list) { /*every service node 's userlist*/
        ...
        // LOG_I(gattc_srv, "[HRSRV]gattc_read_value_cb: -- param_len = %d", param->handle_value->length);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_read_value_cb: -- end");
...
static void gattc_write_value_cb(ble_gattc_write_rsp_t *param)
{
    ...
    conn_id = param->conn_id;
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_value_cb: --start : conn_id = %d", conn_id);
    ...
    if (!srv_node) {
        LOG_I(gattc_srv, "[HRSRV]gattc_write_value_cb: --srv_node is null or char_node is null");
        return ;
    }
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_value_cb: type =%d", type);

    userlist = Get_Head_List(&srv_node->userlist);
    while (userlist != &srv_node->userlist) {/*every service node 's userlist*/

        //LOG_I(gattc_srv, "[HRSRV]gattc_write_value_cb: user_list is not empty =%x", userlist);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_write_value_cb:--end: user =%x", userlist);
}
...
static void gattc_cfg_mtu_cb(ble_gatt_exchange_mtu_rsp_t *param)
{
    ...
    LOG_I(gattc_srv, "[HRSRV]gattc_cfg_mtu_cb:result = %d", param->expected_mtu, param->result);
}
...
static void gattc_report_indication(ble_gattc_handle_value_ind_t *param)
{
    ...
    if (!service_node) {
        LOG_I(gattc_srv, "[HRSRV]gattc_report_indication:---srv_node is null or char_node is null");
        return ;
    }
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_report_indication:--type = %d", type);
    ...
    //LOG_I(gattc_srv, "[HRSRV]gattc_report_indication:--end user_lisr = %x", userlist);
}
...
static void gattc_read_multi_char_value_cb(ble_gattc_read_multiple_char_value_rsp_t *param)
{
    ...
    while (&service_node->userlist != user_list) { /*service node 's all userlist*/
        ...
        memset(&value, 0, sizeof (gattc_value_t));
        memset(&char_info, 0, sizeof (gattc_char_info_struct));
        //LOG_I(gattc_srv, "[HRSRV]gattc_read_value_cb: -- param_len = %d", param->length);
