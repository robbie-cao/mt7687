        ...
        if ( strstr((char*) nmea_param[0], "663") ) {
            if (atoi((char*) nmea_param[1]) == 3) {
                ...
                if (atoi((char*) nmea_param[4]) < 4 || atoi((char*) nmea_param[5]) < 4) {
                    ...
                    GNSSLOGD("Aiding EPO, %s\n");
                    hal_rtc_get_time(&rtc_time);
                    epo_demo_send_assistance_data(rtc_time.rtc_year + 2000,rtc_time.rtc_mon,rtc_time.rtc_day,rtc_time.rtc_hour);
                    #endif
                }
            }
        }
...
void gnss_app_send_cmd_by_other_task(int8_t* buf, uint32_t buf_len)
{
    ...
    message.param2 = buf;
    GNSSLOGD("gnss_app_send_cmd_by_other_task\r\n");
    if (xQueueSend(gnss_task_cntx.gnss_task_queue, &message, 10) == pdPASS) {
        test_flag++;
...
{
    int ret_len = 0;
    GNSSLOGD("gnss_app_send_cmd_by_other_task,buf:%s\r\n", buf);
    while (1) {
...
void gnss_sleep_restart_timer()
{
    ...
    if (g_gnss_location_context.sleep_timer != -1) {
        ...
        g_gnss_location_context.sleep_timer = gnss_start_timer(5, gnss_sleep_timer_handle_func);
    }
}
...
extern int32_t debug_put_data_size;
extern int32_t debug_read_data_size;
...
void gnss_task_msg_handler(gnss_message_struct_t *message)
{
    ...
    GNSSLOGD("gnss_msg_handler, message id:%d, message num:%d, R[%d]P[%d]\n", message->message_id, test_flag--, debug_read_data_size, debug_put_data_size);
