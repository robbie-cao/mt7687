...
unsigned char ATTR_ZIDATA_IN_NONCACHED_RAM_4BYTE_ALIGN frame_buffer[ScrnWidth*ScrnHeight*2];
...
void main_screen_event_handle(int32_t event_id, int32_t param1, void* param2)
{
}
...
void common_event_handler(int32_t event_id, int32_t param1, void* param2)
{
...
static void main_screen_cntx_init()
{
    ...
    font_engine_get_string_width_height(
                                    convert_string_to_wstring(demo_item[0].name), 
                                    strlen(demo_item[0].name),
...
static void main_screen_draw()
{
    ...
    font_engine_show_string(10, 
                            10, 
			    convert_string_to_wstring(DEMO_TITLE_STRING_NAME), 
    ...
    if (main_screen_cntx.has_previous_page) {
        ...
        font_engine_show_string(x, 
                                y, 
                                convert_string_to_wstring(PERVIOUS_PAGE_STRING_NAME), 
    ...
    while (num) {
		...
		int32_t str_len;
		my_itoa(index,pre_index,10);
		str_len = strlen(pre_index);
		pre_index[str_len] = '.';
		pre_index[str_len + 1] = 0;

        font_engine_show_string(x - 30, 
                                y, 
                                convert_string_to_wstring(pre_index), 
                                strlen(pre_index), 
                                main_screen_cntx.color);

		font_engine_show_string(x, 
                                y, 
                                convert_string_to_wstring(demo_item[index].name), 
                                strlen(demo_item[index].name), 
                                main_screen_cntx.color);
    ...
    if (main_screen_cntx.has_next_page) {
        ...
        font_engine_show_string(x, 
                                y, 
                                convert_string_to_wstring(NEXT_PAGE_STRING_NAME), 
