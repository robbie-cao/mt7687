...
{
    int32_t i;
	GNSSLOGD("gnss_timer_init, is_init:%d\n", is_init);
	if (is_init) {
        return;
    }
    ...
    g_gnss_timer = xTimerCreate(    "GNSS Timer",       // Just a text name, not used by the kernel.
                    ...
                    pdTRUE,        // The timers will not auto-reload themselves when they expire.
...
void gnss_timer_deinit()
{
	...
	GNSSLOGD("gnss_timer_deinit, is_init:%d\n", is_init);
    if (!is_init) {
        return;
    }
...
void gnss_run_timer()
{
	...
	GNSSLOGD("gnss_run_timer timer_header:%d, is_gnss_timer_actived:%d\n", timer_header, is_gnss_timer_actived);
...
{
    int32_t ret = 0;
    GNSSLOGD("get_ajust_timer_offset\n");
...
void gnss_ajust_timer()
{
    ...
    //xTimerStop(g_gnss_timer, 0);
	GNSSLOGD("gnss_ajust_timer timer_header:%d\n", timer_header);
    ...
    if (curr_tick >= systick_flag) {
		GNSSLOGD("change timer period, curr duration:%d\n", curr_tick - systick_flag);
        ...
        xTimerChangePeriod(g_gnss_timer, gnss_timer_pool[timer_header].tick_offset - (curr_tick - systick_flag), 10);
    }
}
...
int32_t get_timer_item_from_free_list()
{
    ...
    GNSSLOGD("get_timer_item_from_free_list,free_header:%d\n", free_header);
    return ret;
}

void put_timer_item_to_free_list(int32_t item_ind)
{
    GNSSLOGD("put_timer_item_to_free_list:%d\n", item_ind);
...
void insert_to_timer_list(int32_t item_ind)
{
	...
	GNSSLOGD("insert_to_timer_list\n");
    GNSSLOGD("[IIB]timer_header:%d, excute_header:%d, free_header:%d\n", timer_header, excute_header, free_header);
    ...
    GNSSLOGD("[IIE]timer_header:%d, excute_header:%d, free_header:%d\n", timer_header, excute_header, free_header);
}

void append_excute_time_list(int32_t tid){
    int32_t ind;
	GNSSLOGD("append_excute_time_list,tid:%d, excute header:%d\n", tid, excute_header);
    if (excute_header == -1) {
...
void move_exp_timer_to_excute_list()
{
    ...
    int32_t timer_ind;
    int32_t temp_timer_header = -1;
    int32_t temp_timer = -1;
    GNSSLOGD("move_exp_timer_to_excute_list\n");
    GNSSLOGD("[MMB]timer_header:%d, excute_header:%d, free_header:%d\n", timer_header, excute_header, free_header);
    if (timer_header == -1) { // means not timer id in list
        return;
    }
    ...
    do {
        ...
        if (gnss_timer_pool[removed_timer_header].flag == GNSS_TIMER_ONCE_TYPE) {
            ...
            if (temp_timer_header == -1) {
                temp_timer_header = temp_timer = removed_timer_header;
                gnss_timer_pool[removed_timer_header].next_timer = -1;
            } else {
                gnss_timer_pool[temp_timer].next_timer = removed_timer_header;
                gnss_timer_pool[removed_timer_header].next_timer = -1;
                temp_timer = removed_timer_header;
            }
        }
    ...
    while (temp_timer_header != -1) {
        temp_timer = temp_timer_header;
        temp_timer_header = gnss_timer_pool[temp_timer_header].next_timer;
        gnss_timer_pool[temp_timer].tick_offset = gnss_timer_pool[temp_timer].msec / portTICK_PERIOD_MS;
        insert_to_timer_list(temp_timer);
    }
    GNSSLOGD("[MME]timer_header:%d, excute_header:%d, free_header:%d\n", timer_header, excute_header, free_header);
}
...
void delete_timer_item(int32_t tid)
{
    ...
    GNSSLOGD("delete_timer_item:%d\n", tid);
    GNSSLOGD("[DDB]timer_header:%d, excute_header:%d, free_header:%d\n", timer_header, excute_header, free_header);
    ...
    if (ind != -1) {
        ...
        if (ind != -1) {
            if (ind == timer_header) {
                timer_header = gnss_timer_pool[timer_header].next_timer;
                if (timer_header == -1 
                		|| gnss_timer_pool[timer_header].tick_offset < gnss_timer_pool[timer_header].tick_offset)
                gnss_ajust_timer();
            } else {
                gnss_timer_pool[pre_ind].next_timer = gnss_timer_pool[ind].next_timer;
            }
    ...
    GNSSLOGD("[DDE]timer_header:%d, excute_header:%d, free_header:%d\n", timer_header, excute_header, free_header);
}

int32_t gnss_start_timer_internal(int32_t msec, gnss_timer_callback_f call_back, int32_t mode)
{
    int32_t tid;
    GNSSLOGD("gnss_start_timer_internal,0:%d\n", msec);
    
    tid = get_timer_item_from_free_list();
    
    GNSSLOGD("gnss_start_timer_internal,tid:%d\n", tid);
    if (tid == -1) {
        return tid;
    }
...
{
	int32_t ret;
    GNSSLOGD("gnss_start_repeat_timer:%d\n", msec);
...
{
    GNSSLOGD("GNSS timer callback:%d\n", (int)xTaskGetTickCount());
