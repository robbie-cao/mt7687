...
static void gnss_nmea_data_process(int8_t* nmea_buf, int32_t buf_len) {
    if (strstr((char*) nmea_buf, "GPGGA")) {
        ...
        error = gnss_get_location(&g_gnss_location_context.nmea_sentence, &g_gnss_location_context.location_data);
        if (!error && g_gnss_location_context.location_data.accuracy < GNSS_VALID_ACCURACY) {
...
TaskHandle_t gnss_demo_app_create()
{
    ...
    ret = xTaskCreate( gnss_task_main, GNSS_TASK_NAME, GNSS_TASK_STACK_SIZE, NULL, GNSS_TASK_PRIORITY, &task_handler );
