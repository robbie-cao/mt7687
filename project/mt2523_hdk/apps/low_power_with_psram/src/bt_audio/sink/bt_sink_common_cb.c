...
static atci_status_t bt_sink_hf_atci_init_handler(atci_parse_cmd_param_t *parse_cmd);
#endif /* __BT_SINK_HF_INIT_PARAMS_SWITCH__ */

static atci_cmd_hdlr_item_t bt_sink_atci_cmd[] = {
#ifdef __BT_SINK_HF_INIT_PARAMS_SWITCH__
    {
        .command_head = "AT+BTHFINIT",    /**< AT command string. */
        .command_hdlr = bt_sink_hf_atci_init_handler,
        .hash_value1 = 0,
        .hash_value2 = 0
    },
#endif /* __BT_SINK_HF_INIT_PARAMS_SWITCH__ */
...
void bt_sink_cm_atci_init(void)
{
    ...
    ret = atci_register_handler(bt_sink_atci_cmd, sizeof(bt_sink_atci_cmd) / sizeof(atci_cmd_hdlr_item_t));
...
#ifdef __BT_SINK_HF_INIT_PARAMS_SWITCH__
atci_status_t bt_sink_hf_atci_init_handler(atci_parse_cmd_param_t *parse_cmd)
{
    atci_response_t response = {{0}, 0};
    int16_t result;
    ...
    bt_sink_report("[Sink][HF] INIT:%s", parse_cmd->string_ptr + 12);

    switch (parse_cmd->mode) {
        case ATCI_CMD_MODE_EXECUTION: /* rec: AT+BTHFINIT=<type> */
            result = bt_sink_hf_set_init_params((*(parse_cmd->string_ptr + 12) - '0'));
            if (result == 0) {
                response.response_flag = ATCI_RESPONSE_FLAG_APPEND_OK;
            } else {
                snprintf((char *)response.response_buf,
