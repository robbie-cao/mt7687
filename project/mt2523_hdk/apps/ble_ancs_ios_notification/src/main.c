...
static void ble_at_cmd_processing(void)
{
    while (1) {
        ...
        if (read_len > 0) {
            ...
            LOG_I(ANCS, "[BT_CMD] command len: %d, CMD: %s", read_len, g_bt_cmd_rx_vfifo);
...
static void ble_io_uart_irq(hal_uart_callback_event_t event, void *parameter)
{
    ...
    LOG_I(ANCS, "event = %d, semaphore_HD = 0x%x", event, g_bt_io_semaphore);
...
static void prvSetupHardware( void )
{
    ...
    hal_pinmux_set_function(HAL_GPIO_5,6);

    hal_gpio_init(HAL_GPIO_4);

    hal_pinmux_set_function(HAL_GPIO_4,6);*/
