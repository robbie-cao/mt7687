...
extern bool g_disc_screen_exited;
extern bool g_conn_screen_exited;
extern bool g_range_screen_exited;
...
#ifdef __BT_SINK_WITH_PXP__
                g_disc_screen_exited = false;
                g_conn_screen_exited = false;
                g_range_screen_exited = false;
#endif /* __BT_SINK_WITH_PXP__ */
                show_main_screen();
                touch = 10;
            }
...
static void bt_audio_show_pxp_status(void)
{
    ...
    switch (g_bt_audio_cntx.pxp_status) {
        case BT_SINK_EVENT_EXTERN_PXP_CONNECT:
            string = STR_BT_AUDIO_PXP_CONNECTED_;
            break;

        case BT_SINK_EVENT_EXTERN_PXP_ALERT:
            string = STR_BT_AUDIO_PXP_ALERT_;
            break;

        default:
            string = STR_BT_AUDIO_PXP_IDLE_;
            break;
    }

    font_engine_show_string(BT_AUDIO_PXP_STATUS_X,
...
#ifdef __BT_SINK_WITH_PXP__
bt_sink_status_t bt_audio_pxp_event_hdr(bt_sink_event_id_t event_id, void *parameters)
{
...
#ifdef __BT_SINK_WITH_PXP__
    g_disc_screen_exited = true;
    g_conn_screen_exited = true;
    g_range_screen_exited = true;
    bt_sink_event_register_callback(BT_SINK_EVENT_ALL, bt_audio_sink_event_hdr);
    bt_sink_event_register_callback(BT_SINK_EVENT_EXTERN_PXP_CONNECT, bt_audio_pxp_event_hdr);
    bt_sink_event_register_callback(BT_SINK_EVENT_EXTERN_PXP_DISCONNECT, bt_audio_pxp_event_hdr);
    bt_sink_event_register_callback(BT_SINK_EVENT_EXTERN_PXP_ALERT, bt_audio_pxp_event_hdr);
#endif /* __BT_SINK_WITH_PXP__ */
    bt_sink_event_post(BT_SINK_EVENT_CM_POWER_SWITCH, NULL, NULL);
