 ...
 * Description:
 * ------------
 * This file implements Find Me app main function
 *
 ****************************************************************************/
#include <stdio.h>
#include "syslog.h"
...
typedef struct
{
    ...
    uint8_t  alert_level_is_excute;                                            /**< is excute write(TRUE) or cancel(FALSE). */
...
} ble_gatts_fmp_cntx_t;

/************************************************
*   Global variables
*************************************************/
#define FMP_MAX_MULTIPLE_HANDLE_NUMBER 8
#define FMP_MAX_WRITE_DATA_LENGTH 158

ble_evt_t p_ble_evt_t;
ble_ias_t p_fmp_ias_t;
ble_gatts_fmp_cntx_t g_fmp_cntx_t[MAX_LE_DEV_NUM];
uint16_t read_handle[FMP_MAX_MULTIPLE_HANDLE_NUMBER];
uint8_t write_data[FMP_MAX_WRITE_DATA_LENGTH];
/************************************************
*   static utilities
*************************************************/
static void ble_fmp_evt_dispatch(ble_evt_t *p_ble_evt);
static void ble_fmp_services_init(void);
static void ble_fmp_on_ble_evt(ble_evt_t *p_ble_evt);
//static ble_status_t ble_fmp_advertise_data_init(void);
static uint16_t ble_fmp_get_cntx_index_by_conn_id(uint16_t conn_id);
static uint16_t ble_fmp_get_null_cntx_index(void);
static uint16_t ble_fmp_get_link_number(void);
static bool ble_fmp_have_existed_in_cntx(uint16_t conn_id);

log_create_module(FMP_MAIN, PRINT_LEVEL_INFO);

/************************************************
*   Functions
*************************************************/
void ble_fmp_start_advertise(void)
{
    uint8_t buff[31] = {2, 1, 0x1A};
    char* LE_name = "my_hj_dev"; /* you can rename it*/
    uint8_t len = 0;
    /*you can reassign any address*/
    ble_address_t addr;
    addr.address[0] = 0x03;
    addr.address[1] = 9;
    addr.address[2] = 8;
    ...
    LOG_I(FMP_MAIN,"[BLE_FMP] ble_fmp_start_advertise\r\n");
}
...
static uint16_t ble_fmp_get_cntx_index_by_conn_id(uint16_t conn_id)
{
    ...
    for (i = 0; i < MAX_LE_DEV_NUM; i++) {
        if (conn_id == g_fmp_cntx_t[i].conn_id) {
            index = i;
            break;
        }
    }
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_get_cntx_index_by_conn_id: index[%d]\n", index);
    return index;
}

/**
 * @brief Function for verifying the context of link with connection id is exsited or not.
 */
static bool ble_fmp_have_existed_in_cntx(uint16_t conn_id)
{
    uint16_t i;
    ...
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_have_existed_in_cntx: is_existed[%d],conn_id[%d]\n", is_existed, conn_id);
    return is_existed;
}

/**
 * @brief Function for getting a null context.
 */
static uint16_t ble_fmp_get_null_cntx_index(void)
{
    uint16_t i;
    ...
    for (i = 0; i < MAX_LE_DEV_NUM; i++) {
        if (0 == g_fmp_cntx_t[i].conn_id) {
            index = i;
            break;
        }
    }
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_get_null_cntx_index: null index[%d]\n", index);
    return index;
}

/**
 * @brief Function for getting the linked number which have get cntx.
 */
static uint16_t ble_fmp_get_link_number(void)
{
    uint16_t i;
    ...
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_get_link_number: linked number[%d]\n", number);
    return number;
}
 ...
 */
void ble_gatts_fmp_ias_evt_handler(ble_ias_t *p_ias, ble_ias_evt_t *p_evt)
{
    if (false == ble_fmp_have_existed_in_cntx(p_evt->conn_id)) {
        LOG_I(FMP_MAIN, "[BLE_FMP] ble_gatts_fmp_ias_evt_handler, invalid conn id \n");
        return;
    } else {
        uint16_t conn_id = p_evt->conn_id;
        uint16_t index = ble_fmp_get_cntx_index_by_conn_id(conn_id);
        LOG_I(FMP_MAIN, "[BLE_FMP] ble_gatts_fmp_ias_evt_handler: conn_id[%d]\n", conn_id);

        switch (p_evt->evt_type) {
            case BLE_IAS_ALERT_LEVEL_EVT_WRITE:
            {
                g_fmp_cntx_t[index].ias_cntx.alert_level.alert_level.alert_level = p_evt->params.alert_level.alert_level.alert_level;
                LOG_I(FMP_MAIN, "[BLE_FMP] ble_gatts_fmp_ias_evt_handler alert_level_write: level[%d] \n", p_evt->params.alert_level.alert_level.alert_level);
            }
                break;
...
/**
 * @brief Function for handling the callback of update connection params evnet.
 */
void ble_fmp_connection_param_updated_callback(ble_gap_connection_param_updated_ind_t *conn_updated)
{
    ble_gap_conn_param_priority_t conn_param;
    conn_param = ble_gap_get_current_connection_interval(conn_updated);

    if (true == ble_fmp_have_existed_in_cntx(conn_updated->conn_id)) {
        uint16_t index = ble_fmp_get_cntx_index_by_conn_id(conn_updated->conn_id);
        if (conn_param == g_fmp_cntx_t[index].current_priority) {
            LOG_I(FMP_MAIN, "[BLE_FMP] connection param update successfully \n");
        }
    } else {
        LOG_I(FMP_MAIN, "[BLE_FMP] connection param update fail, invalid conn id \n");
    }
}
 ...
 */
ble_status_t ble_fmp_set_advertising_interval(uint16_t conn_id)
{
    ble_status_t status;
    uint8_t name[8] = {0};
    ble_gap_adv_interval_enum_t interval = BLE_GAP_ADV_INTERVAL_FAST_CONNECT;

    memcpy(name, "fmp", sizeof("fmp"));
    status = ble_gap_set_advertising_interval(conn_id, name, interval);  
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_set_advertising_interval: status[%d]\n", status);
    return status;
}

/**
 * @brief Function for updating the connection interval.
 */
ble_status_t ble_fmp_update_connection_interval(uint16_t conn_id)
{
    ble_status_t status;
    uint8_t name[8] = {0};
    ...
    if (status == BLE_STATUS_GAP_PENDING) {
        uint16_t index = ble_fmp_get_cntx_index_by_conn_id(conn_id);
        g_fmp_cntx_t[index].current_priority = interval;
    }

    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_update_connection_interval: status[%d]\n", status);
    return status;
}
...
static void ble_fmp_on_ble_evt(ble_evt_t *p_ble_evt)
{
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_on_ble_evt, evt_id: %d\n", p_ble_evt->header.evt_id);
    switch (p_ble_evt->header.evt_id) {
        case BLE_POWER_ON_CNF:
        {
            if (BLE_STATUS_SUCCESS == p_ble_evt->evt.gap_evt.power_on.result) {
                LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_on_ble_evt, power on\r\n");
                ble_fmp_main();
                ble_fmp_start_advertise();
            }
        }
            break;
        case BLE_POWER_OFF_CNF:
        {
            if (BLE_STATUS_SUCCESS == p_ble_evt->evt.gap_evt.power_off.result) {
                LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_on_ble_evt, power off\r\n");
            }
        }
            break;
        case BLE_GAP_CONNECTED_IND:
        {
            if (ble_fmp_get_link_number() < MAX_LE_DEV_NUM) {
                uint16_t conn_id = p_ble_evt->evt.gap_evt.connected.conn_id;
                if (false == ble_fmp_have_existed_in_cntx(conn_id)) {
                    uint16_t index = ble_fmp_get_null_cntx_index();
                    memset(&g_fmp_cntx_t[index], 0, sizeof(ble_gatts_fmp_cntx_t));
                    g_fmp_cntx_t[index].conn_id = conn_id;

                    LOG_I(FMP_MAIN, "[BLE_FMP] BLE_GAP_CONNECTED_IND: conn_id[%d]\n", conn_id);
                }
            }
        }
            break;
        ...
        {
            if (true == ble_fmp_have_existed_in_cntx(p_ble_evt->evt.gap_evt.disconnected.conn_id)) {
                uint16_t index = ble_fmp_get_cntx_index_by_conn_id(p_ble_evt->evt.gap_evt.disconnected.conn_id);
                memset(&g_fmp_cntx_t[index], 0, sizeof(ble_gatts_fmp_cntx_t));

                LOG_I(FMP_MAIN, "[BLE_FMP] BLE_GAP_DISCONNECTED_IND: conn_id[%d]\n", p_ble_evt->evt.gap_evt.disconnected.conn_id);
            }
        }
            break;
        ...
        case BLE_GAP_CONNECTION_PARAM_UPDATED_IND:
            ble_fmp_connection_param_updated_callback(&(p_ble_evt_t.evt.gap_evt.conn_params));
            break;
...
static ble_status_t ble_fmp_advertise_data_init(void)
{
    ...
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_advertise_data_init \n");
...
static void ble_fmp_services_init(void)
{
    ...
    memset(&p_fmp_ias_t, 0, sizeof(ble_ias_t));
    ias_init_t.evt_handler = ble_gatts_fmp_ias_evt_handler;
    err_code = ble_ias_init(&p_fmp_ias_t, &ias_init_t);
    if (0 != err_code) {
        LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_services_init ias init: err_code[%d] \n", err_code);
    }
}
 ...
 */
void ble_fmp_main(void)
{
    /**< Initialize. */
    ble_fmp_services_init();
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_main \n");
}
...
static void ble_fmp_evt_dispatch(ble_evt_t *p_ble_evt)
{
    ...
    ble_ias_on_ble_evt(&p_fmp_ias_t, p_ble_evt);
    ble_fmp_on_ble_evt(p_ble_evt);
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_fmp_evt_dispatch \n");
}
...
{
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_gatts_message_handler: message_id[%d] \n", message_id);
    ...
    switch (message_id) {	
        ...
        case BLE_MSG_GATTS_READ_REQ_IND:
        {
            ...
            memset(read_handle, 0, FMP_MAX_MULTIPLE_HANDLE_NUMBER);
            if (temp_req->count <= FMP_MAX_MULTIPLE_HANDLE_NUMBER) {
                memcpy(read_handle, temp_req->handle, (sizeof(uint16_t) * temp_req->count));
            } else {
                memcpy(read_handle, temp_req->handle, (sizeof(uint16_t) * FMP_MAX_MULTIPLE_HANDLE_NUMBER));
            }
        ...
        case BLE_MSG_GATTS_WRITE_REQ_IND:
        {
            ...
            memset(write_data, 0, FMP_MAX_WRITE_DATA_LENGTH);   
            if (temp_data->length <= FMP_MAX_WRITE_DATA_LENGTH) {
                memcpy(write_data, temp_data->data, (sizeof(uint8_t) * temp_data->length));
            } else {
                memcpy(write_data, temp_data->data, (sizeof(uint8_t) * FMP_MAX_WRITE_DATA_LENGTH));
            }
    ...
    ble_fmp_evt_dispatch(&p_ble_evt_t);	
}
...
{
    LOG_I(FMP_MAIN, "[BLE_FMP] ble_gap_message_handler: message_id[%d] \n", message_id);
