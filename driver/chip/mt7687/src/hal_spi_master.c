...
{
    /* GPIO32 for slave 0  GPIO6 for slave 1 */
    return (HAL_SPI_MASTER_SLAVE_0 == slave_port) ? HAL_GPIO_32 : HAL_GPIO_6;
}
        ...
        uint32_t size)
{
    ...
    hal_gpio_set_output(hal_spi_master_map_cs_pin(g_spi_master_context.slave_port), HAL_GPIO_DATA_LOW);
#ifdef HAL_SLEEP_MANAGER_ENABLED
...
#endif
    hal_gpio_set_output(hal_spi_master_map_cs_pin(g_spi_master_context.slave_port), HAL_GPIO_DATA_HIGH);
