...
void connsys_get_ilm_image_info(fw_dl_data_t *output, ssize_t length)
{
    ...
    log_hal_info( "ilm_addr = %08x, ilm_len = %x, feature_set = %d.\n",
        (unsigned int)info->ilm_info.addr,
        (unsigned int)info->ilm_info.len,
        (int)info->ilm_info.feature_set);
    log_hal_info( "dlm_addr = %08x, dlm_len = %x, feature_set = %d.\n",
...
void connsys_get_ilm_image_info_from_flash(fw_dl_data_t *output)
{
    ...
    log_hal_dump("tailer", (const char *)n9_fw_info, sizeof(fw_image_tailer_t));
...
uint32_t connsys_img_direct_download(uint32_t target_addr, uint32_t length, uint32_t src_addr, uint32_t data_mode, uint32_t key_index)
{
    ...
    log_hal_info( "Before Switch PDA source, con=0x%x, state=0x%x\n", volatile_reg1, volatile_reg2);
    // Step2: Set PDA source to PKT-GEN, instead of HIF
    REG32(PDA_SOURCE_CON) |= (PDA_SOURCE_SEL_BIT);
    volatile_reg1 = REG32(PDA_SOURCE_CON);
    volatile_reg2 = REG32(PDA_DWLD_STATE);
    log_hal_info( "After Switch PDA source, con=0x%x, state=0x%x\n", volatile_reg1, volatile_reg2);
...
//        REG32(0xE000E100) = 1;
        ...
        // wait DMA finish
        log_hal_info( "Wait GDMA finishing...\n");
        ...
        log_hal_info( "GDMA move from (0x%x) %d bytes to N9, total %d bytes.\n",
    ...
    if (total != length)
    {
        log_hal_fatal( "ERROR! the total %d bytes != assigned length(%d bytes)\n",
...
int32_t connsys_util_fw_direct_download(void)
{
    ...
    {
        log_hal_info( "<<%s>> Get ownership ok.\n", __FUNCTION__);
    }
    else
    {
        log_hal_info( "<<%s>> Get ownership failed.\n", __FUNCTION__);
    }
    ...
    log_hal_info( "DLM encrypt(%d), dlm key index(%d)\n",
        (int)data.dlm_encrypt,
        (int)data.dlm_encrypt_key_index);


    // Step3: download ILM
    log_hal_info( "Download ILM (%d bytes to N9 0x%x, mode:0x%x)...\n",
    ...
    if (ptr == NULL)
    {
        log_hal_fatal( "Download ILM Fail.\n");
        return 0;
    }

    // Step4: download DLM
    log_hal_info( "Download DLM (%d bytes to N9 0x%x, mode:0x%x), ptr=0x%p...\n",
    ...
    if (ptr == NULL)
    {
        log_hal_fatal( "Download DLM Fail.\n");
        return 0;
    }
...
void connsys_fw_download(fw_dl_data_t *data)
{
    ...
    log_hal_info( "<<%s>> The Bin File , ilm_addr = %08x, ilm_tx_len = %x\n",
            __FUNCTION__,  (unsigned int)data->ilm_addr, (unsigned int)data->ilm_len);

    log_hal_info( "<<%s>> dlm_addr = %08x, dlm_tx_len = %x, enrypt = %d, encrypt_key_index = %d\n",
            __FUNCTION__,
    ...
    log_hal_info( "<<%s>> ILM: Send fw download command...\n", __FUNCTION__);
    ...
    log_hal_info( "<<%s>> DLM: Send fw download command...\n", __FUNCTION__);
...
uint8_t connsys_patch_finish(void)
{
    ...
    while (1)
    {
        ...
        if (ret)
        {
            log_hal_fatal( "<<%s>> Read WHISR failed. Error = %d.\n", __FUNCTION__, (int)ret);
            return err;
        }
        ...
        {
            log_hal_fatal( "<<%s>> Check interrupt failed. wait_tx_done = %d, wait_tx_done = %d.\n",
                __FUNCTION__,
                (int)wait_tx_done,
                (int)wait_rx_done);
            return err;
    ...
    log_hal_info( "<<%s>> Rx Patch finish event...\n", __FUNCTION__);
    ...
    {
        log_hal_fatal( "<<%s>> Patch finish response length incoreect. WRPLR = %08x.\n",
            __FUNCTION__,
            (unsigned int)value);
        return err;
    }

    log_hal_info( "<<%s>> Receive Patch finish event ...\n", __FUNCTION__);

    connsys_fifo_read(WRDR0, buffer, real_rx_len);

    // print_content(real_rx_len, buffer, "patch finish..2");
    log_hal_dump("patch finish..2", (const char *)buffer, real_rx_len);
...
uint32_t connsys_fw_download_CmdAddressLenReq(uint32_t addr, uint32_t len, uint32_t data_mode, uint8_t *buffer)
{
    ...
    log_hal_dump("FW download: addr len req", (const char *)buffer, cmd_len);
...
uint8_t connsys_util_fw_download_CmdAddressLenReq_event_check(uint8_t *buffer)
{
    ...
    log_hal_dump("FW download: addr len req evt check", (const char *)buffer, p_event->u2RxByteCount);

    if ((p_status->ucStatus) || (p_event->ucEID != INIT_EVENT_ID_CMD_RESULT))
    {
        log_hal_fatal( "<<%s>> Download config event incorrect. EID = %d, Status = %d\n",
            __FUNCTION__,
            (int)p_event->ucEID,
    ...
    {
        log_hal_info( "<<%s>> Download config set Successfully.\n", __FUNCTION__);
    }
...
uint8_t connsys_util_patch_download_get_semaphore_event_check(uint8_t *buffer, uint8_t *status)
{
    ...
    log_hal_dump("Patch download", (const char *)buffer, p_event->u2RxByteCount);

    if (p_event->ucEID != INIT_EVENT_ID_PATCH_SEMA_CTRL)
    {
        log_hal_fatal( "<<%s>> Patch semaphore event incorrect. EID = %d\n",
            __FUNCTION__,
            (int)p_event->ucEID);
        err = 1;
    }
    ...
    {
        log_hal_info( "<<%s>> Patch semaphore event ok.\n", __FUNCTION__);
...
int32_t connsys_fw_download_Tx_Scatter(uint8_t *image, uint32_t len)
{
    ...
    while (left_len)
    {
        ...
        log_hal_info( "<<%s>> Read file total_len: %d.\n", __FUNCTION__, (int)pos);
        // print_content(32, g_tx_buf, "Send FW (first 32 bytes)");
        log_hal_dump("Send FW (first 32 bytes)", (const char *)g_tx_buf, 32);
        ...
        {
            // error cuures
            log_hal_fatal( "<<%s>> Read scatter failed. Error = %d.\n", __FUNCTION__, (int)ret);
            err = 1;;
        }
    }
    return err;
...
int32_t connsys_pda_config(uint32_t dest_addr, uint32_t data_len, uint32_t data_mode)
{
    ...
    log_hal_info( "<<%s>> Send fw download command...\n", __FUNCTION__);

    os_memset(buffer, 0, SDIO_MAX_RW_SIZE);
    ...
    // rx event from dut
    log_hal_info( "<<%s>> Rx fw download event...\n", __FUNCTION__);
    ret = connsys_cr_read(WRPLR, &value);
    if (ret)
    {
        log_hal_fatal( "<<%s>> Read WRPLR failed. Error = %d.\n", __FUNCTION__, (int)ret);
    ...
    {
        log_hal_fatal( "<<%s>> download config event response length incoreect. WRPLR = %08x.\n",
            __FUNCTION__,
            (unsigned int)value);
        return err;
    }
...
void connsys_util_fw_download_done()
{
    ...
    log_hal_dump("fw download test", (const char *)buffer, tx_len);

    // send to dut
    log_hal_info( "<<%s>> Send FW_START command ...\n", __FUNCTION__);
    // write connsys_util_fw_download cmd to dut
    ret = connsys_fifo_write(buffer, tx_len);
    if (ret != 0)
    {
    ...
    if (GET_RX0_PACKET_LENGTH(value) != real_rx_len)
    {
        log_hal_fatal( "<<%s>> FW start response length incoreect. WRPLR = %08x.\n",
            __FUNCTION__,
            (unsigned int)value);
        return;
    }

    log_hal_info( "<<%s>> Receive FW_START event ...\n", __FUNCTION__);

    connsys_fifo_read(WRDR0, buffer, real_rx_len);
    // print_content(real_rx_len, buffer, "fw download test..2");
    log_hal_dump("fw download test..2", (const char *)buffer, real_rx_len);
    ...
    while (1)
    {
        ...
        if (ret)
        {
            log_hal_fatal( "<<%s>> Read WCIR failed. Error = %d.\n", __FUNCTION__, (int)ret);
            return;
        }

        log_hal_info( "<<%s>> WCIR = %08x.\n", __FUNCTION__, (unsigned int)value);
    ...
    log_hal_info( "<<%s>> FW Download Successfully.\n", __FUNCTION__);
}
...
uint8_t connsys_get_patch_semaphore(void)
{
    ...
    while (get_semaphore)
    {
        ...
        log_hal_dump("get patch semaphore", (const char *)buffer, tx_len);

        // send to dut
        log_hal_info( "<<%s>> Send Get Patch Semaphore command ...\n", __FUNCTION__);
        // write connsys_util_fw_download cmd to dut
        ret = connsys_fifo_write(buffer, tx_len);
        if (ret != 0)
        {
        ...
        {
            log_hal_fatal( "<<%s>> Get patch semaphore response length incoreect. WRPLR = %08x.\n",
                __FUNCTION__,
                (unsigned int)value);
            return err;
        }

        log_hal_info( "<<%s>> Receive Get patch semaphore event ...\n", __FUNCTION__);

        connsys_fifo_read(WRDR0, buffer, valid_rx_len);
        // print_content(valid_rx_len, buffer, "get patch semaphore...2");
        log_hal_dump( "get patch semaphore...2", (const char *)buffer, valid_rx_len);
        ...
        else
        {
            if (status == WIFI_PATCH_SEMA_NEED_PATCH_SEMA_FAIL)
            {
                ...
                {
                    log_hal_fatal( "<<%s>> FAIL. Get patch semaphore fail.\n", __FUNCTION__);
                    return err;
                }
...
void connsys_patch_direct_download()
{
...
#if 0
    log_hal_info( "<<%s>> The Bin File = %s, ilm_addr = %08x, ilm_tx_len = %x\n",
            __FUNCTION__, data->file_name, data->ilm_addr, data->ilm_len);
#endif

    data.image = (uint8_t *)(WIFI_FW_ADDR_IN_FLASH);
    data.ilm_addr = MT7637_PATCH_START_ADDR;
    ...
    {
        log_hal_info( "<<%s>> Send patch download command...\n", __FUNCTION__);
        ...
        if (ptr == NULL)
        {
            log_hal_fatal( "Download PATCH Fail.\n");
            return ;
        }
        ...
        log_hal_info( "<<%s>> Patch Direct Download Successfully.\n", __FUNCTION__);
    }
    else if (status == WIFI_PATCH_SEMA_PATCH_DONE_SEMA_IGNORE)
    {
        log_hal_info( "<<%s>> Patch has been done, needn't get semaphore.\n", __FUNCTION__);
    }
    else if (status == WIFI_PATCH_SEMA_RELEASE_SUCCESS)
    {
        log_hal_info( "<<%s>> Fail! Get semaphore instead of release patch semaphore.\n", __FUNCTION__);
    }
    else if (status == WIFI_PATCH_SEMA_NEED_PATCH_SEMA_FAIL)
    {
        log_hal_info( "<<%s>> Fail! Get patch semaphore failed.\n", __FUNCTION__);
    }
...
void connsys_patch_download_body(const patch_dl_data_t *data)
{
    ...
    if (status == WIFI_PATCH_SEMA_NEED_PATCH_SEMA_SUCCESS)
    {
        ...
        log_hal_info( "<<%s>> Patch download Successfully.\n", __FUNCTION__);
    }
    else if (status == WIFI_PATCH_SEMA_PATCH_DONE_SEMA_IGNORE)
    {
        log_hal_info( "<<%s>> Patch has been done, needn't get semaphore.\n", __FUNCTION__);
...
void connsys_util_patch_download(void)
{
	uint32_t fw_size;
    patch_dl_data_t data = {
    	.ilm_addr = MT7637_PATCH_START_ADDR,
    	.ilm_len  = 0,
    	.image    = NULL,
    };
    log_hal_info( "==> HW ID=0x%x\n", REG32(CM4_CONFIG_BASE));
	...
	//log_hal_info( "==> PATCH size: %d bytes, build time: %s\n", (int)data.ilm_len,	g_patch_info->built_date);

    log_hal_info( "patch ilm_addr = %08x, ilm_len = %x.\n", (unsigned int)data.ilm_addr, (unsigned int)data.ilm_len);
    connsys_patch_download_body(&data);
}

#if 0
#if 1
#include "mt7637_firmware.h"
#else
uint8_t MT7637_FirmwareImage[]= {0x00}; // for test, avoid LARGE array lead to memory overflow
#endif
...
void connsys_util_fw_download()
{
    ...
    data.image = MT7637_FirmwareImage;
    connsys_get_ilm_image_info(&data, sizeof(MT7637_FirmwareImage));

    // add 4 byte CRC len
    data.ilm_len += LEN_4_BYTE_CRC;
    data.dlm_len += LEN_4_BYTE_CRC;

    log_hal_info( "ilm_addr = %08x, ilm_len = %x, encryt = %d, key_index = %d.\n", data.ilm_addr, data.ilm_len, data.ilm_encrypt, data.ilm_encrypt_key_index);
    log_hal_info( "dlm_addr = %08x, dlm_len = %x.\n", data.dlm_addr, data.dlm_len);
...
int32_t connsys_util_tx_data(uint8_t *buf, ssize_t len)
{
    ...
    if (CONNSYS_TEST_DEBUG(CONNSYS_DBG_TX_Q0) || CONNSYS_TEST_DEBUG(CONNSYS_DBG_TX_Q1))
    {
        ...
        {
            log_hal_dump("P1_Q0 tx_buf", (uint8_t *)buf, (uint32_t)len);
        }
        else
        {
            log_hal_dump("P1_Q1 tx_buf", (uint8_t *)buf, (uint32_t)len);
        }
    }
...
int32_t connsys_util_rx_data(uint8_t *buf, ssize_t len, int32_t port)
{
    ...
    if (ret)
    {
        log_hal_fatal( "<<%s>> Failed. Rx data, port(0x%x)\n", __FUNCTION__, WRDR1);
...
int32_t connsys_util_rx_data_enhance_mode(uint8_t *buf, ssize_t *len, int32_t port)
{
    ...
    else
    {
        ...
        {
            log_hal_fatal( "FAIL. write RX_ENHANCE_MODE fail. WHCR = 0x%08x.\n", reg_value);
            return CONNSYS_STATUS_FAIL;
        }
    }
    log_hal_info( "Enable enhance mode, WHCR=0x%x\n", reg_value);
...
void connsys_intr_enhance_mode_receive_one_data(int32_t port, int16_t rx_len)
{
    ...
    else
    {
        ...
        if ((continuous_allocate_fail_cnt > CFG_CONNSYS_CONTINOUS_ALLOCATE_FAIL_PRINT_CNT_VAL)&&
            ...
            log_hal_error( "WARN! connsys: can't allocate buffer for %u times\n",
    ...
    g_connsys_callback_func.input_pkt_callback(pkt_ptr, port, rx_len);
...
int32_t connsys_util_intr_enhance_mode_receive_data(void)
{
    ...
    //while (retry<rtry_level)
    {
        ...
        {
            if (!retry) {
                log_hal_error("WHISR=0x%x, abort...\n", (unsigned int)cr_whisr);
            }
        ...
        if (cr_whisr & ABNORMAL_INT)
        {
            ...
            log_hal_error("Abnormal Interrupt: WASR(0x%x), WASR2(0x%x)\n", (unsigned int)reg1, (unsigned int)reg2);
        }
        ...
        if (cr_whisr & FW_OWN_BACK_INT)
        {
            ...
            //log_hal_info("FW_OWN_BACK_INT: WASR(0x%x)\n", (unsigned int)reg1);
        }
    #endif
...
#include "wifi_scan.h" // for debug only, remove it later
#endif
int32_t connsys_util_get_inf_number(uint8_t *buf)
{
    ...
    if (__g_wpa_supplicant_api.wpa_supplicant_entry_op_mode_get) {
        ...
        if (op_mode == WIFI_MODE_P2P_ONLY) { // force to go to inf-1 due to N9 FW not ready
            header->aucReserved[0] = 0;
            header->aucReserved[0] |= IOT_PACKET_TYPE_INF_0;
        }
    }
...
int32_t connsys_emulLP_connsys_open()
{
    ...
    if (ret)
    {
       log_hal_fatal( "<<%s>> Enable function failed. Error = %d.\n", __FUNCTION__, (int)ret);
       goto err;
    }
    ...
    if (ret)
    {
        log_hal_fatal( "<<%s>> Set block size failed. Error = %d.\n", __FUNCTION__, (int)ret);
        goto err;
    }
    ...
    if (ret)
    {
        log_hal_fatal( "<<%s>> Claim irq failed. Error = %d.\n", __FUNCTION__, (int)ret);
        goto err;
    }
...
//    NVIC_EnableIRQ(CM4_HIF_IRQ);
    ...
    //log_hal_info( "write 0x160 done, ret=0x%x\n", ret);

    log_hal_info( " End of %s\n", __FUNCTION__);
err:
    return ret;
}
...
{
    log_hal_info( "set debug flag %u\n", (unsigned int)debug);
    if (debug <= CONNSYS_DBG_MAX_NUM)
        CONNSYS_SET_DEBUG((1<<debug));
    else
        log_hal_info( "WARN! wrong argument. connsys has no such debug flag.\n");
    }

void connsys_cli_clear_debug_falg(uint32_t debug)
{
    log_hal_info( "clear debug flag %u\n", (unsigned int)debug);

    if (debug <= CONNSYS_DBG_MAX_NUM)
        CONNSYS_CLEAR_DEBUG((1<<debug));
    else
        log_hal_error( "WARN! wrong argument. connsys has no such debug flag.\n");
    }
...
{
    log_hal_info( "==>connsys_util_cli_handler\n");
    ...
    switch (atoi(param[0]))
    {
        ...
        case 1:
        {
            ...
            if (len != 2)
                log_hal_error( "WARN! wrong argument\n");
            else
