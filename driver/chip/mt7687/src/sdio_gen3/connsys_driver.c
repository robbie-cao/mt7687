...
struct connsys_callback_func g_connsys_callback_func;
...
void connsys_measure_time_show(void)
{
    ...
    local_irq_save(flags);
    os_memcpy(&tmp, &g_connsys_time_last, sizeof(connsys_measure_time_t));
    local_irq_restore(flags);
    ...
    local_irq_save(flags);
    os_memcpy(&tmp, &g_connsys_time_max, sizeof(connsys_measure_time_t));
    local_irq_restore(flags);
...
void connsys_print_stat(void)
{
    ...
    local_irq_save(flags);
    os_memcpy(&stat, &g_connsys_stat, sizeof(connsys_stat_t));
    local_irq_restore(flags);
...
int32_t connsys_cccr_read(uint32_t addr, uint8_t *value)
{
	...
	*value = connsys_bus_fn0_read_byte(dev_func, addr, &ret);
	...
	if (ret)
    {
        LOG_E(hal, "<<%s>> Read CCCR 0x%02x failed. Error = %d\n",
            __FUNCTION__,
            (unsigned int)addr,
            (int)ret);
	}
...
int32_t connsys_cccr_write(uint32_t addr, uint8_t value)
{
    ...
    if (ret)
    {
        LOG_E(hal, "<<%s>> Write register 0x%02x failed. Error = %d\n",
            __FUNCTION__,
            (unsigned int)addr,
            (int)ret);
    }
...
int32_t connsys_cr_read(uint32_t addr, uint32_t *value)
{
	...
	*value = connsys_bus_readl(dev_func, addr, &ret);
	...
	if (ret)
    {
        LOG_E(hal, "<<%s>> Read register 0x%08x failed. Error = %d\n",
            __FUNCTION__,
            (unsigned int)addr,
            (int)ret);
	}
...
int32_t connsys_cr_write(uint32_t addr, uint32_t value)
{
    ...
    if (ret)
    {
        LOG_E(hal, "<<%s>> Write register 0x%08x failed. Error = %d\n",
            __FUNCTION__,
            (unsigned int)addr,
            (int)ret);
    }
...
int32_t connsys_fifo_read(uint32_t addr, uint8_t *buf, size_t size)
{
	...
	if (ret)
    {
		LOG_E(hal, "<<%s>> SDIO read data failed. Error = %d \n", __FUNCTION__, (int)ret);
...
{
    rx_interrupt_handle = (void(*)(int32_t))pfunc;
}

void connsys_driver_interrupt(struct connsys_func *func)
{

    if(rx_interrupt_handle != NULL)
        rx_interrupt_handle(0);
...
uint8_t connsys_get_ownership(void)
{
    ...
    if (ret)
    {
        LOG_E(hal, "Ownership is already driver\n");
        goto err;
    }
    ...
    if (!GET_W_FW_OWN_REQ_SET(value))
    {
        ...
        if (ret)
        {
            LOG_E(hal, "request owner ship write fail\n");
            goto err;
        }
...
int32_t connsys_abnormal_interrupt_check(uint32_t value)
{
    ...
    if (value & ABNORMAL_INT)
    {
        ...
        if (err)
        {
            LOG_E(hal, "<<%s>> Read WASR failed. Error = %d.\n", __FUNCTION__, (int)err);
        }
        else
        {
            LOG_I(hal, "<<%s>> Read WASR = %08x.\n", __FUNCTION__, (unsigned int)reason);
        }
    }
    return err;
}
...
void connsys_irq_handler(hal_nvic_irq_t irq_number)
{
    ...
    if ((p_int_enhance->WHISR_reg_val) & ABNORMAL_INT)
	{
        ...
        local_irq_restore(flags);
		//LOG_E(hal, "Abnormal(0x%x), 0xE0(0x%x)\n", (unsigned int)reg1, (unsigned int)reg2);
	}
    ...
    if ((p_int_enhance->WHISR_reg_val) & FW_OWN_BACK_INT)
    {
        ...
        local_irq_restore(flags);
		//LOG_E(hal, "FW_OWN_BACK_INT: WASR(0x%x)\n", (unsigned int)reg1);
    }
#endif
...
#endif
        local_irq_save(flags);
        os_memcpy(&g_last_enhance_mode_data_struct, input, LEN_INT_ENHANCE_MODE);
...
int32_t connsys_enable_enhance_mode(void)
{
    ...
    LOG_I(hal, "enable connsys enhance mode.\n");

    if (connsys_cr_read(WHCR, &reg_value))
    {
        LOG_E(hal, "FAIL. read WHCR.\n");
		return -1;
    }
...
//    reg_value |= RX_ENHANCE_MODE;
    ...
    LOG_I(hal, "write 0x%x to WHCR\n", (unsigned int)reg_value);

    if (connsys_cr_write(WHCR, reg_value))
    {
        LOG_E(hal, "FAIL. write WHCR.\n");
        return -1;
    }
    reg_value = 0;
    if (connsys_cr_read(WHCR, &reg_value))
    ...
    else
    {
        ...
        {
            LOG_E(hal, "FAIL. write RX_ENHANCE_MODE fail. WHCR = 0x%08x.\n", (unsigned int)reg_value);
            return -1;
        }
#endif
    }
//    LOG_I(hal, "Enable enhance mode, WHCR=0x%x\n", (unsigned int)reg_value);
...
int32_t connsys_open()
{
    ...
    LOG_I(hal, "============> SDIO open, (0x%p) use DMA(%d)\n",
    ...
    if (ret)
    {
       LOG_E(hal, "<<%s>> Enable function failed. Error = %d.\n", __FUNCTION__, (int)ret);
       goto err;
    }
    ...
    if (ret)
    {
        LOG_E(hal, "<<%s>> Set block size failed. Error = %d.\n", __FUNCTION__, (int)ret);
        goto err;
    }
    ...
    if (ret)
    {
        LOG_E(hal, "<<%s>> Claim irq failed. Error = %d.\n", __FUNCTION__, (int)ret);
        goto err;
    }
    ...
    if(connsys_get_ownership() == FALSE) {
        LOG_E(hal, "connsys_get_ownership failed.\n");
        ret = -1;
        goto err;
    }
    if(connsys_enable_enhance_mode() != 0) {
        LOG_E(hal, "connsys_enable_enhance_mode failed.\n");
        ret = -1;
        goto err;
    }
    ...
    {
        LOG_E(hal, "FAIL. write WHIER failed (1).\n");
        ret = CONNSYS_STATUS_FAIL;
        goto err;
    }
    else
    {
        ...
        {
            LOG_E(hal, "FAIL. WHISR.RX0/1_DONE interrupt should be cleared first. (2). WHISR = 0x%08x.\n", (unsigned int)reg_value);
            ret = CONNSYS_STATUS_FAIL;
            goto err;
        }
        reg_value = 0;
        ...
        if (!(reg_value & (RX0_DONE_INT_EN | RX1_DONE_INT_EN)))
        {
            LOG_E(hal, "FAIL. write WHIER failed (2). WHIER = 0x%08x.\n", (unsigned int)reg_value);
            ret = CONNSYS_STATUS_FAIL;
            goto err;
        }
    }
...
int32_t connsys_close()
{
    ...
    if (ret)
    {
        LOG_E(hal, "<<%s>> Release irq failed. Error = %d.\n", __FUNCTION__, (int)ret);
    }
    ...
    if (ret)
    {
        LOG_E(hal, "<<%s>> Disable function failed. Error = %d.\n", __FUNCTION__, (int)ret);
    }

    LOG_I(hal, "<<%s>> Yes. Release Done.\n", __FUNCTION__);
...
void connsys_tx_flow_control_init(void)
{
    uint32_t flags;
    local_irq_save(flags);
    os_memset(&g_hif_tx_flow_control_stat, 0, sizeof(wifi_hif_tx_flow_control_t));
...
{
    uint32_t flags;
    LOG_I(hal, "==>connsys_tx_flow_control_config, hif_tx_flow_ctrl_en = %u\n",
...
void connsys_tx_flow_control_set_reserve_page(uint32_t reserve_page)
{
    uint32_t flags;
    wifi_hif_tx_flow_control_t* ctrl = &g_hif_tx_flow_control_stat;
    LOG_I(hal, "==>connsys_tx_flow_control_set_reserve_page, reserve_page = %u\n",
...
void connsys_tx_flow_control_show_info(void)
{
    ...
    local_irq_save(flags);
    os_memcpy(&wifi_hif_tx_flow_control,
...
{
    g_connsys_callback_func.allocate_pkt_callback = func;
}

void connsys_register_input_pkt_callback(connsys_input_pkt_t func)
{
    g_connsys_callback_func.input_pkt_callback = func;
}
...
int8_t lp_connsys_get_own_enable_int()
{
    ...
    if (connsys_get_ownership()) {
        //printf("FAIL. get connsys ownership.\n");
        // TODO: return 1;
    }

    /* enable FW_OWN_BACK_INT interrupt */
    if (connsys_cr_write(WHIER, 0x00000080)) {
        printf("FAIL. write WHIER failed (1).\n");
        return 1;
    } else {
        reg_value = 0;
        connsys_cr_read(WHIER, &reg_value);
        if (reg_value != 0x00000080) {
            printf("FAIL. write WHIER failed (2). WHIER = 0x%08x (expected = 0xFFFFFF00)\n", (unsigned int)reg_value);
            return 1;
        }
    }
    ...
    if (connsys_cr_write(WHLPCR, W_INT_EN_SET)) {
        printf("FAIL. write WHLPCR failed.\n");
        return 1;
...
int8_t lp_connsys_give_n9_own()
{
    ...
    if (connsys_cr_read(WHLPCR, &reg_value)) {
        printf("FAIL. read WHLPCR failed.\n");
        return 1;
    }
...
int32_t connsys_init(sys_cfg_t *sys_config)
{
    ...
    LOG_W(hal, "WARN! LWIP DMA data copy disabled..\n");
