	...
	int32_t size)
{
    ...
    {
        LOG_E(hal, "==>connsys_bus_read_port, align error, dst = 0x%x", (unsigned int)dst);
    ...
    */
    ...
    /*
		if (drop_count >  0)
		{
			...
			LOG_E(hal, "DMA mode drop count(%d) not support yet!", (int)drop_count);
		}
    */
    }
    else
...
{
    LOG_I(hal,"info: 0x%x", (unsigned int)info->word);
    LOG_I(hal,"\tByte/Block Count: %u", info->field.count);
    LOG_I(hal,"\tRegister Address: 0x%x", info->field.addr);
    LOG_I(hal,"\tOP Code: 0x%x", info->field.op_mode);
    LOG_I(hal,"\tBlock Mode: 0x%x", info->field.block_mode);
    LOG_I(hal,"\tFunction Number: 0x%x", info->field.func_num);
    LOG_I(hal,"\tR/W flag: 0x%x", info->field.rw_flag);
}
	...
	int32_t size)
{
    ...
    {
        LOG_HEXDUMP_E(hal, "connsys_bus_write_port, align error, packet: ", (uint8_t *)src, (uint32_t)size);
        return CONNSYS_STATUS_FAIL;
    }
...
{
    uint32_t i;
    LOG_I(hal, "dump tx scatter info:");
    LOG_HEXDUMP_I(hal, "txd", tx_info->txd, 12);
    LOG_I(hal, "reserved_headroom_len = %u, tx_len = %u, real_tx_len = %u, buf_num = %u, fg_wmm = %d",
    ...
    {
        LOG_I(hal, "[%u]: ", (unsigned int)i);
        LOG_HEXDUMP_I(hal, "content", tx_info->buff[i].buf_ptr, tx_info->buff[i].buf_len);
    }
}
...
int32_t connsys_write_data(connsys_tx_scatter_info_t* tx_info)
{
    ...
    if (remain_reserved_headroom_len && (!func->use_dma))
    {
        ...
        n9_pad_buf = 0;
        os_memcpy((void *)((uint32_t)((uint8_t *)&n9_pad_buf)+ remain_reserved_headroom_len),
    ...
    for (buf_idx = 0; buf_idx < tx_info->buf_num; buf_idx++)
    {
        ...
        if (func->use_dma)
        {
            ...
            if (!IS_ALIGN_4((uint32_t)src))
            {
                ...
                LOG_I(hal, "[DMA] 4-align addr(%d), len(%d), src:0x%x, tx_info->reserved_headroom_len(%d), remain_reserved_headroom_len(%d), src offset 2 bytes...\n",
        ...
        if (!IS_ALIGN_4((uint32_t)src))
            LOG_E(hal, "[TX] 4-byte align(%d), src:0x%x\n", IS_ALIGN_4((uint32_t)src), src);
#endif
...
int32_t connsys_bus_enable_func(struct connsys_func *func)
{
	...
	LOG_I(hal, "SDIO: Enabling Function %d...\n", (int)func->num);

    reg = connsys_bus_fn0_read_byte(func, SDIO_CCCR_IOEx, &ret);
	if (ret)
		goto err;
    LOG_I(hal, "Origin Func enable=0x%x\n", reg);
	reg |= 1 << func->num;
	...
	if (ret)
		goto err;
    LOG_I(hal, "Read CCCR_IORx=0x%x\n", reg);
	if (!(reg & (1 << func->num)))
    {
    	ret = -ETIME;
        goto err;
    }
	LOG_I(hal, "SDIO: Enabled Function %d\n", (int)func->num);

	return 0;

err:
	LOG_I(hal,"SDIO: Failed to enable Function %d", (int)func->num);
	return ret;
}
/**
...
int32_t connsys_bus_disable_func(struct connsys_func *func)
{
    ...
    LOG_I(hal, "SDIO: Disabling Function %d...", (int)func->num);
    ...
    LOG_I(hal, "SDIO: Disabled Function %d", (int)func->num);

    return 0;

err:
    ret = -EIO;
    LOG_I(hal, "SDIO: Failed to Disable Function %d", (int)func->num);
    return ret;
...
int32_t connsys_bus_get_irq(struct connsys_func *func, connsys_irq_handler_t *handler)
{
	...
	LOG_I(hal, "SDIO: Enabling IRQ for func%d...\n", (int)func->num);

	if (func->irq_callback) {
		LOG_E(hal, "SDIO: IRQ for func%d already in use.\n", (int)func->num);
		return -2; //-EBUSY;
	}
...
int32_t connsys_bus_release_irq(struct connsys_func *func)
{
	...
	LOG_I(hal, "SDIO: Disabling IRQ for func%d...", (int)func->num);
