...
========================================================================
*/
static low_hal_irrx_callback_t s_low_hal_irrx_rc5_callback;
static low_hal_irrx_callback_t s_low_hal_irrx_pwd_callback;
...
static VOID halIrRxIsr(hal_nvic_irq_t irq_number)
{
    ...
    if (((pIRRXTypeDef->IRCFGH) & (BIT(IRCFGH_CHK_EN_OFFSET)))) {
        ...
        if (s_low_hal_irrx_pwd_callback) {
            s_low_hal_irrx_pwd_callback();
        }
    } else {
        // RC5
        if (s_low_hal_irrx_rc5_callback) {
...
ENUM_HAL_RET_T halIrRxRegisterCallback(low_hal_irrx_mode_t mode, low_hal_irrx_callback_t callback)
{
    ENUM_HAL_RET_T  ret = HAL_RET_SUCCESS;

    if (LOW_HAL_IRRX_MODE_RC5 == mode) {
        s_low_hal_irrx_rc5_callback = callback;
    } else if (LOW_HAL_IRRX_MODE_PWD == mode) {
        s_low_hal_irrx_pwd_callback = callback;
    }
