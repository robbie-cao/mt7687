...
#endif
#ifdef HAL_DVFS_MODULE_ENABLED
static dvfs_lock_t uart_dvfs_lock = {
    .domain = "VCORE",
    .addressee = "UART_DVFS",
};
static bool g_uart_dvfs_lock_status[HAL_UART_MAX];
#endif
...
#endif
#ifdef HAL_DVFS_MODULE_ENABLED
                /*unlock dvfs after transfer end.*/
                if (g_uart_dvfs_lock_status[uart_port] == true) {
                    dvfs_unlock(&uart_dvfs_lock);
                    g_uart_dvfs_lock_status[uart_port] = false;
                }
#endif
            }
...
#ifdef HAL_SLEEP_MANAGER_ENABLED
ATTR_TEXT_IN_TCM void uart_backup_all_registers(void)
{
    /* do nothing for the moment. */
}

ATTR_TEXT_IN_TCM void uart_restore_all_registers(void)
{
    uint32_t delay;
...
#endif
#ifdef HAL_DVFS_MODULE_ENABLED
            g_uart_dvfs_lock_status[i] = false;
#endif
...
uint32_t hal_uart_send_polling(hal_uart_port_t uart_port, const uint8_t *data, uint32_t size)
{
    ...
    irq_status = save_and_set_interrupt_mask();
    if (g_uart_dvfs_lock_status[uart_port] == false) {
        dvfs_lock(&uart_dvfs_lock);
        g_uart_dvfs_lock_status[uart_port] = true;
    }
    restore_interrupt_mask(irq_status);
#endif
