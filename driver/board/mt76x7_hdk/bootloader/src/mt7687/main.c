...
static void user_select(void)
{
    ...
    while (1) {
        ...
        switch (ch) {
        case '1':
            fotu_do(FOTA_PARITION_LOADER, FLASH_LOADER_SIZE);
            break;

        case '2':
            fotu_do(FOTA_PARITION_CM4, FLASH_CM4_XIP_CODE_SIZE);
            break;

        case '3':
            fotu_do(FOTA_PARITION_NCP, FLASH_N9_RAM_CODE_SIZE);
            break;
...
#if defined(__GNUC__)
    pc_addr = (CM4_FLASH_CM4_ADDR + 0x10000000) | 0x1; /* switch to ARM state */
    jump_to(pc_addr, sp_addr);
//#elif defined(__CC_ARM)
#else
    Jump_Cmd((CM4_FLASH_CM4_ADDR + 0x10000000));
#endif
