 ...
 * @{
 * This section defines the Low Energy (LE) GAP confirmation/indication macros, structures and API prototypes.
 ...
 * - Then it receives a GAP confirmation/indication. The GAP confirmation/indication is used to notify the application that the process is complete or the indication is received.
 ...
 * - The GAP confirmation/indication structures provide the related information to the application.
 ...
 *          }
 *
 *          // This function is a static callback for the application to listen to the event. It is a user-defined callback.
 *          bt_status_t bt_app_event_callback(bt_msg_type_t msg, bt_status_t status, void *buff)
 *          {
 *              switch(msg)
 *              {
 *                  case BT_GAP_LE_BONDING_INFO_REQ_IND:
 *                      // Assign a blank memory for bonding information if it has not been bonded with the connected device or assign with the right bonding information to the stack.
 *                      // If this message is ignored, it will lead to an assertion error.
 *                      BT_LOGI("APP", "BT_GAP_LE_BONDING_INFO_REQ_IND %s", (status == BT_STATUS_SUCCESS)? "Successful":"Failed");
 *                      ((bt_gap_le_bonding_info_req_ind_t *)buff)->bonding_info_req = &(get_bonded_info(&(((bt_gap_le_bonding_info_req_ind_t *)buff)->remote_addr), 1)->info);
 *                      break;
 ...
 *                      // Indicate the status of executing bt_gap_le_disconnect() API.
...
/**
 * @defgroup hbif_gap_define Define
 * @{
 */
...
#define BT_GAP_LE_SMP_AUTH_REQ_NO_BONDING              0x00        /**< Do not save the IRK, LTK and CSRK. */
#define BT_GAP_LE_SMP_AUTH_REQ_BONDING                 (0x01 << 0) /**< Save the IRK, LTK and CSRK. */
#define BT_GAP_LE_SMP_AUTH_REQ_MITM                    (0x01 << 2) /**< Using the MITM protection. */
#define BT_GAP_LE_SMP_AUTH_REQ_SECURE_CONNECTION       (0x01 << 3) /**< Using LE secure connection. */
...
#define BT_GAP_LE_LOCAL_CONFIG_REQ_IND             (BT_MODULE_GAP | 0x0002)    /**< Local Configuration Request Indication with #bt_gap_le_local_config_req_ind_t applied right after #BT_POWER_ON_CNF.
                                                                                    The application should not call any APIs in this event and the #bt_gap_le_local_config_req_ind_t should not be NULL. */
...
#define BT_GAP_LE_DISCONNECT_IND                   (BT_MODULE_GAP | 0x000d)    /**< Disconnect Indication with NULL payload, indicates a connection is disconnected. */
...
#define BT_GAP_LE_CONNECTION_UPDATE_REQ_IND        (BT_MODULE_GAP | 0x0010)    /**< Connection Update Request Indication with #bt_gap_le_connection_update_req_ind_t. */
...
#define BT_GAP_LE_BONDING_INFO_REQ_IND             (BT_MODULE_GAP | 0x0013)    /**< Bonding Information Request Indication with #bt_gap_le_bonding_info_req_ind_t applied right after #BT_GAP_LE_CONNECT_IND. The application should return the memory address of #bt_gap_le_bonding_info_t without calling any APIs.
                                                                                    The bonding information should not be NULL. */
#define BT_GAP_LE_BONDING_START_IND                (BT_MODULE_GAP | 0x0014)    /**< Bonding Start Indication with #bt_gap_le_bonding_start_ind_t, indicates a pairing procedure has started. Should not call any APIs. */
...
/**
 * @defgroup hbif_gap_struct Struct
 * @{
 */
...
typedef struct {
    ...
    uint8_t                            maximum_encryption_key_size;    /**< Maximum size of an encryption key. The recommended value is 16. */
    bt_gap_le_smp_key_distribute_t     initiator_key_distribution;     /**< The key sent from the Central device. */
    bt_gap_le_smp_key_distribute_t     responder_key_distribution;     /**< The key sent from the Peripheral device. */
...
typedef struct {
    ...
    bt_gap_le_security_mode_t           key_security_mode;  /**< Security mode #bt_gap_le_security_mode_t. */
    uint8_t                             key_size;           /**< Key size. */
}) bt_gap_le_bonding_info_t;

/**
 *  @brief Local key information.
 */
BT_PACKED (
typedef struct {
    bt_gap_le_encryption_info_t         encryption_info; /**< Encryption information. */
...
typedef struct {
    ...
    bt_bool_t                  sc_only_mode_req; /**< Set this variable to enable the secure connection only mode. */
 ...
 */
typedef struct {
    const bt_addr_t            remote_addr;       /**< The address of the remote device to be bonded. */
...
/**
 *  @brief Connection indication, an event sent to the application once the connection is established.
 ...
 */
typedef struct {
    uint16_t        interval_min;            /**< Range[6..0xc80], Time = N * 1.25 msec. */
    uint16_t        interval_max;            /**< Range[6..0xc80], Time = N * 1.25 msec. */
...
/**
 *  @brief Indication to update the connection parameters, an event sent to
 *         the application after receiving the peer connection update parameters
 *         to inform the application to validate the parameters.
 */
typedef struct {
    bt_hci_accept_t                        accept_req; /**< Accept or reject the connection update. */
    bt_gap_le_connection_update_param_t    param_req;  /**< The connection parameters received from the remote device. The application can decide whether to update or not. */
} bt_gap_le_connection_update_req_ind_t;

/**
 *  @brief Connection update indication, an event sent to the application after
 *         the controller has already updated the connection parameters to
 *         indicate the connection is updated.
 */
typedef struct {
    bt_handle_t     conn_handle;         /**< Handle of the connection that is updated. */
...
typedef bt_hci_evt_disconnect_complete_t bt_gap_le_disconnect_ind_t;  /**< Disconnect indication, an event sent to the application after the connection was already disconnected. */
...
/**
 *  @brief Bonding complete indication.
 */
typedef struct {
...
/**
 *  @brief Indication to request bonding reply.
 */
typedef struct {
    bt_handle_t                    handle;              /**< Connection handle. */
...
typedef struct {
    union {
        ...
        bt_bool_t   nc_value_matched; /**< Set to #BT_TRUE, if the displayed value is a match when doing a numeric comparison. */
    };
} bt_gap_le_bonding_reply_t;

/**
 *  @brief Advertising report indication, an event sent to the application after
 *         receiving the advertising report from the remote device.
 *         Enable the scan to receive an advertising report.
 *         Please refer to #bt_gap_le_set_scan() API.
 */
BT_PACKED (
typedef struct {
    bt_gap_le_adv_report_evt_type_t event_type; /**< Event type. */
    bt_addr_t       address;                 /**< The address of the advertiser sending the advertisement. */
    bt_addr_t       direct_address;          /**< The address the directed advertisement is being directed to. */
 ...
 * @param[in] addr          is the address with type to be set.
 *                          It should not be NULL when using #BT_GAP_LE_ADD_TO_WHITE_LIST or #BT_GAP_LE_REMOVE_FROM_WHITE_LIST.
 *                          And, it should be NULL when using #BT_GAP_LE_CLEAR_WHITE_LIST operation.
 * @return    #BT_STATUS_SUCCESS, the operation completed successfully, otherwise it failed.
 */
bt_status_t bt_gap_le_set_white_list(bt_gap_le_set_white_list_op_t op, const bt_addr_t *addr);
 ...
 * @return    #BT_STATUS_SUCCESS, the operation completed successfully, otherwise it failed.
 */
bt_status_t bt_gap_le_set_address_resolution_enable(bt_bool_t enable);
 ...
 *            The appliation receives the #BT_GAP_LE_BONDING_START_IND event after the pairing procedure starts and
 *            the #BT_GAP_LE_BONDING_COMPLETE_IND event after the pairing procedure is complete.
