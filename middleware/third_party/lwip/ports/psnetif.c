...
#define ATCMD_MAX_RAW_DATA_SIZE   1000   //ATCMD_MAX_PKT_SIZE substract  //AT+EIPDATA=<CID>,<Length>,<
...
#define PS_NETIF_TASK_STACKSIZE      (1024)
#define PS_NETIF_TASK_PRIO           (3)  // The more the number, the higher the priority in FreeRTOS
...
extern int32_t netif_remove_all();
...
low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  char *end_data = "\"\r\n";
  if (is_sending == 1) {
    LWIP_DEBUGF(NETIF_DEBUG, ("low_level_output:is sending now\n"));
    return ERR_OK;
  }
  ...
  data = pvPortMalloc(ATCMD_EIPDATA_HEADER + total_len * 2 + 1);
  if (data == NULL) {
    return ERR_BUF;
  }
  snd_data = pvPortMalloc(total_len + 1);
  if (snd_data == NULL) {
    vPortFree(data);
    return ERR_BUF;
  }
  ...
  ret_len = sio_send_data(tcpip_app_id, (uint8_t *)data, len);
  is_sending = 1;
  ...
  ret_len = data->len;
  #else
  ret_len = (ret_len != len) ? ERR_BUF : ERR_OK;
  #endif
  vPortFree(snd_data);
  vPortFree(data);
  return ret_len;
...
void ps_netif_task_process(void *arg)
{
    ...
    for (;;) {
        if (xQueueReceive(ps_netif_queue_id, &msg_queue_data, portMAX_DELAY) == pdPASS) {
            ...
            /* message handling */
            if (MSG_ID_PS_NETIF_CMUX_DATA == msg_queue_data.msg_id) {
                low_level_input_cmux_data((cmux_event_ready_to_read_t *)(msg_queue_data.msg_data));
            } else if (MSG_ID_PS_NETIF_UART_DATA == msg_queue_data.msg_id) {
...
ps_netif_init(void)
{
  ...
  lwip_socket_init();
  tcpip_init(NULL, NULL); //tcpip_init(tcpip_init_done, tcpip_init_done_param);
