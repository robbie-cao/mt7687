...
#define HTTPD_PROC_THREAD_NAME "httpd_proc"
#define HTTPD_PROC_THREAD_STACKSIZE 2500
#define HTTPD_PROC_THREAD_PRIO 0  // The more the number, the higher the priority in FreeRTOS
#define HTTPD_PROC_QUEUE_SIZE 1 // Only Message can be handle
...
HTTPD_RESULT httpd_init(void)
{
    ...
    {
        httpd_proc_mbox = xQueueCreate(HTTPD_PROC_QUEUE_SIZE, sizeof(httpd_para));
        if(httpd_proc_mbox == NULL)
        {
    ...
    if (httpd_proc_task_handle == NULL)
    {
        xReturn = xTaskCreate(
            httpd_proc_thread,
            HTTPD_PROC_THREAD_NAME,
            HTTPD_PROC_THREAD_STACKSIZE/sizeof(portSTACK_TYPE),
            NULL,
            HTTPD_PROC_THREAD_PRIO,
