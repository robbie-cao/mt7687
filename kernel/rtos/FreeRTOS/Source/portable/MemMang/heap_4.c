...
#else
	ATTR_ZIDATA_IN_NONCACHED_RAM static uint8_t ucHeap[ configTOTAL_HEAP_SIZE ] ;
...
void vCheckAccessRegion(void* addr, size_t size)
{
	...
	while (blk_iter != pxEnd)
	{
		...
		if (addr >= (void *)blk_iter + sizeof(BlockLink_t) && addr < (void *)blk_iter + blk_size)
		{
			if(addr + size > (void *)blk_iter + blk_size)
			{
				printf("heap use overflow!!!! task = %s\n", pcTaskGetTaskName(xTaskGetCurrentTaskHandle()));
				while(1);
			}
		}
		blk_iter = (void *)blk_iter + blk_size;
	}
	taskEXIT_CRITICAL();
}
...
void vDumpHeapStatus()
{
	...
	{
		blk_size = blk_iter->xBlockSize & ~xBlockAllocatedBit;
		printf("block start = 0x%x,\t size = 0x%x,\t task = %s\n", (unsigned int)blk_iter, blk_size, (blk_iter->xBlockSize & xBlockAllocatedBit) ? pcTaskGetTaskName(blk_iter->xHandle) : " ");
		blk_iter = (void *)blk_iter + blk_size;
		if((unsigned int)blk_iter > (unsigned int)pxEnd){
			printf("heap crash!!!!!\n");
			while(1);
		}
	}
